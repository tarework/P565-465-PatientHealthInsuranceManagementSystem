...
function ValidatePatientMedicalData(request) {
    const schema = Joi.object({
        ...
        birthdate: Joi.string().regex(constants.birthdate).required().error(()=>new Error('Height format is invalid.')),
        ...
        smokefreq: Joi.number(),
        drink: Joi.boolean().required(),
        drinkfreq: Joi.number(),
        caffeine: Joi.boolean().required(),
        caffinefreq: Joi.number()
    });
